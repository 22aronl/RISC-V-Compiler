...
enum class Type { ASSIGN, COPY, JUMP, IF, IF_FALSE, PARAM, CALL, POINTER, POINTEE, RETURN };
...
class Node { // three address representation
  ...
  public:
    ...
    Node(Type type, std::string l_value, int jump_label) : type(type), l_value(l_value), jump_label(jump_label) { r_node = new R_Node("", "", ""); }

    void add_label(int label) { labels.push_back(label); }
...
class Environment {
  public:
    ...
    int get_icg_size() { return icg_nodes.size(); }

    int get_label_count() { return label_count; }

    std::vector<Node *> get_icg_nodes() { return icg_nodes; }

    MemOperand *get_var_location(std::string reg_name) {
        // TODO: write code to find the offset needed
        return nullptr;
    }
};
